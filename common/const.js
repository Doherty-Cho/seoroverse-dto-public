...
const filterValidationObject = Object.fromEntries(
  Object.entries(FILTER_CONDITION).map(([, v]) => [
    v,
    lazy((obj) => {
      ...
      return v === 'in'
        ? arrayTypeSchemaForFilter(
            Array.isArray(obj) && obj.length > 0 ? getSchemaFromObj(obj[0]) : string().nullable()
          )
...
function hasFilterConditionKeys(targetObj) {
  if (!targetObj || !isObject(targetObj) || isEmpty(targetObj)) return false
