...
function validateUnknownKeys(schemaObj, targetObj, field) {
    ...
    if (unknownKeys.length)
        throw new ValidationError(`Unknown key${unknownKeys.length > 1 ? 's' : ''}: ${unknownKeys.join(', ')}`, targetObj, field, unknownKeys[0]);
}
...
const sortObj = lazy((obj) => object(mapValues(obj, () => string().oneOf(Object.values(SORT_OPTION)))));
/**
...
function getFieldsSchema(resourceSchema, obj) {
    ...
    for (const key of Object.keys(obj)) {
        ...
        for (const opr of Object.values(FILTER_CONDITION)) {
            if (Object.prototype.hasOwnProperty.call(val, opr) &&
                (val[opr] === null ||
                    val[opr] === undefined ||
 ...
 */
const getSearchSchema = (resourceSchema) => lazy((obj) => {
    if (isEmpty(obj)) {
        return object().notRequired();
    }
